...
export function buildFlight(){
 ...
 const apogee=frames.reduce((a,b)=>b.h>a.h?b:a),displaySeparationAltitude=100000,descentPeak=displaySeparationAltitude*.9;
 ...
 return {displaySeparationAltitude,apogeeTime:apogee.t,apogeeAltitude:apogee.h,descentPeak,eventTimes,separationTime,separationAltitude:separation.h,frames,duration:frames.at(-1).t,maxQ,maxQTime,entryTime,landingTime,landed,contactTime,impactSpeed};
}
...
if(t<flight.separationTime)s.h*=flight.displaySeparationAltitude/flight.separationAltitude;
if(t>=flight.separationTime&&s.h>0){const u=Math.min(1,(t-flight.separationTime)/(flight.apogeeTime-flight.separationTime)),k=flight.descentPeak*flight.apogeeAltitude/(flight.apogeeAltitude-flight.descentPeak);s.h=t<flight.apogeeTime?flight.displaySeparationAltitude+(flight.descentPeak-flight.displaySeparationAltitude)*u*u*(3-2*u):s.h/(1+s.h/k);}
return s;}
