...
for(let i=1;i<f.frames.length-1;i++){
 const a=f.frames[i-1],b=f.frames[i];if(b.t<155||b.contact)continue;
 assert(Math.abs(b.attitude-a.attitude)<=.14*DT+1e-8,'Angular rate must bound every rotation step');
 assert(Math.abs(b.angularVelocity-a.angularVelocity)<=.13*DT+1e-6,'Angular momentum cannot reset');
